...
# project: drought synthesis (fishes)
# purpose: gather FMWT indices for selected fishes and evaluate change associated with differing year types (Critical, Below Normal, etc) + year (ie change over time)
# note "regimes": year < 1988 "pre-clam", 1988-2001 "pre-pod", 2002+ "pod")
# adopted 31 October 2022
...
# print results ----
# NOTE! won't work until you run the code below ca line 42
...
print(
  fmwt_trs_long %>% 
        group_by(species, yr_type) %>%
        summarise("mean ln(index)" = mean(ln_index, na.rm = T)), 
      n=Inf) 

# using index
print(
  fmwt %>% 
  pivot_longer(cols = threadfin_shad:striped_bass_age0, 
               names_to = "species", 
               values_to = "catch") %>% 
  group_by(species, yr_type) %>% 
  summarise(mean = mean(catch, na.rm = T)) %>% 
...
# 1) For every species, mean ln(index) for AN & Wet were higher (if  not "significantly" so) than other year types.
...
plot(ams3)
summ(ams3, model.info = T, digits = 4) # strong linear effect of year type
...
print("AMS: random, no pattern, no runs (ie no year effect); convincing drought effect")
print(runs_ams)
print(summ(ams3, model.info = T, digits = 4))
print(pw_ams)
sink()
...
summ(ds3, model.info = F, digits = 4)
...
print(runs_ds)
print(summ(ds3, model.info = F, digits = 4))
print(pw_ds)
sink()
...
print(runs_lfs)
print(summ(lfs3, model.info = T, digits = 4))
print(pw_lfs)
sink()
...
summ(sb03, model.info = F, digits = 4) # year & year type...what to make of quadratic model result?
...
")
print(summ(sb03, model.info = F, digits = 4))
print(pw_sb0)
print(runs_sb0)
sink()
...
layout(matrix(c(1:4), 2, 2, byrow = T))
plot(tfs3)
summ(tfs3, digits = 4) # sig year effect (POD crash)
...
print(summ(tfs3, model.info = F, digits = 4))
print(pw_tfs)
print(runs_tfs)
sink()
...
summ(ds3, model.info = T, digits = 4) # makes sense: "trend" is like falling of a cliff; of course no linear model is going to describe this ts well!
